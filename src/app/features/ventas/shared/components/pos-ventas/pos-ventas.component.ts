  ...
  animations: [
    ...
    trigger('slideUpInOut', [
      transition(':enter', [
        ...
        ), // Estado final: posición normal. Usamos un cubic-bezier para un efecto "físico" suave.
      ]),
      transition(':leave', [
        animate(
          '300ms cubic-bezier(0.25, 0.8, 0.25, 1)',
          style({ transform: 'translateY(100%)' }),
        ), // Al salir, vuelve abajo
      ]),
    ]),
...
export class PosVentasComponent implements OnInit, OnDestroy {
  ...
  // Usuario y Sistema
  currentUser = ''; // Se inicializara en ngOnInit
  ventasHoy = 125487;
   ...
   */
  private showProductAddedNotification(
    producto: Inventario,
    cantidad: number,
  ): void {
    this.toastService.productAdded(
   ...
   */
  private showSaleCompletedNotification(
    total: string,
  ...
  private actualizarCantidadesEnTiempoReal(query?: string): void {
    ...
    if (
      ahora - this.ultimaActualizacion <
      this.INTERVALO_MINIMO_ACTUALIZACION
    ) {
    ...
    const inventarioIds = this.productosAutoComplete
      .map((p) => p.id)
    ...
    this.inventarioService
      .obtenerInventarios(0, 30, 'producto.nombre', 'asc', filtros)
      .subscribe({
        next: (response) => {
          // Actualizar cantidades de productos existentes en la lista
          this.productosAutoComplete = this.productosAutoComplete
            .map((producto) => {
              const inventarioActualizado = response.contenido.find(
                (inv) => inv.id === producto.id,
              );
  ...
  private cargarProductosRecientes(): void {
    this.inventarioService
      .obtenerInventarios(0, 10, 'id', 'desc', {
  ...
  private validarPreciosCarrito(): boolean {
    const itemsSinPrecio = this.carrito.filter(
  ...
  notificarErrorPago(mensaje: string): void {
    this.toastService.error('💳 Error en el Pago', mensaje, {
      persistent: true,
  ...
  ngOnInit() {
    ...
    this.currentUser = user ? user.username : 'Vendedor';
  ...
  private inicializarActualizacionInventario(): void {
    ...
    console.log(
      '✅ Sistema de actualización automática de inventario iniciado con optimizaciones',
    );
  }
  ...
  abrirDialogoAperturaCaja(): void {
    this.aperturaCajaRef = this.dialogService.open(
    ...
    this.cierreCajaRef.onClose.subscribe((result) => {
      if (result) {
        ...
        if (result.diferencia === 0) {
          ...
          this.toastService.success('✅ Cierre de Caja', mensaje, {
            duration: 5000,
            icon,
          });
        } else if (diferencia <= 10) {
          mensaje += ` - Diferencia: S/ ${result.diferencia.toFixed(2)}`;
          this.toastService.warning('⚠️ Cierre de Caja', mensaje, {
            duration: 5000,
            icon,
          });
        } else {
          mensaje += ` - Diferencia significativa: S/ ${result.diferencia.toFixed(2)}`;
          this.toastService.error('❌ Cierre de Caja', mensaje, {
            duration: 7000,
            icon,
          });
        }
  ...
  imprimirComprobante(venta: VentaResponse): void {
    console.log('═══════════════════════════════════════════════════════════');
    console.log(
      '🖨️ [INICIO] imprimirComprobante() - Impresión directa en ticketera',
    );
    ...
    if (!venta) {
      ...
      this.toastService.error(
        '❌ Error',
        'No se puede imprimir: Venta no proporcionada',
      );
      return;
    }
  ...
  private mostrarOpcionesImpresion(venta: VentaResponse): void {
    ...
    console.log(
      '🔍 confirmationService disponible:',
    ...
    try {
      ...
      this.toastService.error(
        '❌ Error',
        'No se pudo mostrar opciones de impresión',
      );
    }
  }
  ...
  testImpresion(): void {
    ...
    alert(
      '¡Botón funciona! ventaParaComprobante: ' +
        (this.ventaParaComprobante ? 'EXISTE' : 'NULL'),
    );
  }
  ...
  private async imprimirEnTicketera(venta: VentaResponse): Promise<void> {
    try {
      ...
      this.toastService.info(
        '⏳ Preparando',
        'Generando ticket para impresión...',
        { duration: 2000 },
      );
      ...
      this.toastService.error(
        '❌ Error de Impresión',
        'No se pudo imprimir en ticketera',
      );
   ...
   */
  private async enviarATicketeraViaBackend(
    venta: VentaResponse,
  ): Promise<void> {
    try {
      ...
      this.comprobantesService.verificarConexionTicketera().subscribe({
        ...
        error: (error) => {
          console.warn(
            '⚠️ Error verificando conexión, intentando impresión:',
            error,
          );
  ...
  private continuarConImpresion(venta: VentaResponse): void {
    ...
    this.comprobantesService.obtenerComprobantePorVenta(venta.id).subscribe({
      next: (comprobante) => {
        console.log(
          '✅ Comprobante encontrado, enviando a ticketera:',
          comprobante.id,
        );
      ...
      error: (error) => {
        if (error.status === 404) {
          ...
          this.toastService.error(
            '❌ Error',
            'No se pudo obtener el comprobante para impresión',
          );
        }
      },
    });
    ...
    this.comprobantesService.imprimirEnTicketera(comprobanteId).subscribe({
      next: (response) => {
        ...
        if (response.success) {
          this.toastService.success(
            ...
            response.message ||
              'El ticket se ha enviado correctamente a la XPrinter XP-V320M',
            { duration: 4000 },
          );
      ...
      error: (error) => {
        ...
        if (error.message?.includes('conectar')) {
          mensaje =
            'Verifique que la XPrinter XP-V320M esté conectada y encendida';
        }

        this.toastService.error('❌ Error de Conexión', mensaje, {
          duration: 5000,
        });
  ...
  private cortarPapelTicketera(): void {
    this.comprobantesService.cortarPapel().subscribe({
      next: (response) => {
        if (response.success) {
          this.toastService.success('✂️ Papel Cortado', response.message, {
            duration: 2000,
          });
        } else {
          this.toastService.warning('⚠️ Aviso', response.message, {
            duration: 3000,
          });
        }
      },
  ...
  private imprimirEnImpresoraNormal(venta: VentaResponse): void {
    ...
    this.toastService.info('⏳ Preparando', 'Generando PDF para impresión...', {
      duration: 2000,
    });
    ...
    this.comprobantesService.obtenerComprobantePorVenta(venta.id).subscribe({
      next: (comprobante) => {
        this.comprobantesService.descargarPDF(comprobante.id).subscribe({
          next: (blob) => {
            this.abrirPDFParaImprimir(
              blob,
              `comprobante-${venta.numeroVenta}.pdf`,
            );
          },
          error: (error) => {
            ...
            this.toastService.error(
              '❌ Error',
              'No se pudo preparar el PDF para impresión',
            );
          },
        });
      },
  ...
  private abrirPDFParaImprimir(blob: Blob, nombreArchivo: string): void {
    try {
      ...
      if (ventana) {
        ...
        this.toastService.info(
          '📁 Descargado',
          'Archivo descargado para imprimir manualmente',
        );
      }
  ...
  private generarTicketHTML(comprobanteId: number): void {
    ...
    this.toastService.info(
      '🔄 Alternativa',
      'Preparando ticket en formato web...',
    );
  }
  ...
  enviarComprobantePorEmail(venta: VentaResponse): void {
    ...
    const email =
      (venta.cliente as { email?: string }).email || 'cliente@ejemplo.com';
  ...
  private inicializarComponente() {
    ...
    console.log(
      `🚀 POS iniciado por ${this.currentUser} - ${this.getCurrentDateTime()}`,
    );
  }
  ...
  buscarProductoPorCodigo() {
    ...
    this.inventarioService
      .obtenerInventarios(0, 10, 'id', 'asc', {
        producto: this.codigoBusqueda.trim(),
      })
      .subscribe({
        next: (response) => {
          const productoEncontrado = response.contenido.find(
            (inv) =>
              inv.producto?.codigo?.toLowerCase() ===
                this.codigoBusqueda.trim().toLowerCase() ||
              inv.serie?.toLowerCase() ===
                this.codigoBusqueda.trim().toLowerCase(),
          );
          ...
          if (productoEncontrado && productoEncontrado.cantidad > 0) {
            ...
            const precioUnitario =
              Number(productoEncontrado.producto?.precioVenta) || 0;
  ...
  buscarProductosAutoComplete(event: { query: string }) {
    ...
    this.inventarioService
      .obtenerInventarios(0, 30, 'producto.nombre', 'asc', {
        producto: query,
        soloStockCritico: false,
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.messageService.add({
            ...
            summary: 'Error de Búsqueda',
            detail:
              'No se pudieron cargar los productos. Verifique la conexión.',
          });
          this.cdr.markForCheck();
        },
      });
  ...
  agregarAlCarrito(inventario: InventarioPOS, cantidad: number) {
    ...
    setTimeout(() => {
      const existeEnCarrito = this.carrito.find(
        (item) => item.inventarioId === inventario.id,
      );
      ...
      if (existeEnCarrito) {
        ...
        if (nuevaCantidad <= inventario.stock) {
          ...
          existeEnCarrito.subtotal =
            existeEnCarrito.cantidad * existeEnCarrito.precioUnitario;
        ...
        if (cantidad <= inventario.stock) {
          ...
          if (precioUnitario === 0 && inventario.producto?.id) {
            const precioDirecto = this.obtenerPrecioProducto(
          ...
          const nuevoItem: ItemCarrito = {
            ...
            color: {
              ...
              codigo:
                inventario.color?.nombre?.substring(0, 2).toUpperCase() || 'SC',
            },
            talla: {
  ...
  calcularTotales() {
    ...
    const totalConIGV = this.carrito.reduce(
      (sum, item) => sum + item.subtotal,
      0,
    );

    // Aplicar descuento al total
    this.descuentoVenta = this.aplicarDescuento
      ? (totalConIGV * this.porcentajeDescuento) / 100
      : 0;
  ...
  seleccionarCliente(cliente: Cliente) {
    ...
    if (cliente?.id) {
      ...
      this.notificarClienteSeleccionado(
  ...
  iniciarPago(): void {
    ...
    if (this.carrito.length === 0) {
      ...
      this.mostrarError(
        'Carrito vacío',
        'Debe agregar productos antes de procesar el pago',
      );
      return;
    }
    ...
    if (!this.nuevaVenta.serieComprobante) {
      ...
      this.mostrarError(
        'Serie requerida',
        'Debe seleccionar una serie de comprobante',
      );
      return;
    }
  ...
  private calcularPorcentajeCambio(
    valorActual: number,
  ...
  private cargarProductosPopulares() {
    ...
    this.inventarioService
      .obtenerInventarios(0, 20, 'cantidad', 'desc', {
        soloStockCritico: false,
        soloAgotados: false,
      })
      .subscribe({
  ...
  descargarComprobantePDF(venta: VentaResponse): void {
    if (!venta?.id) {
      this.toastService.error(
        '❌ Error',
        'No se puede descargar el comprobante: Venta inválida',
      );
      return;
    }
    ...
    console.log(
      '🔽 Iniciando descarga PDF para venta:',
      venta.id,
      'Archivo:',
      nombreArchivo,
    );
    ...
    this.toastService.info(
      '⏳ Descargando',
      `Generando archivo PDF: ${nombreArchivo}`,
      { duration: 2000 },
    );
    ...
    this.comprobantesService.obtenerComprobantePorVenta(venta.id).subscribe({
      ...
      error: (error) => {
        ...
        if (esComprobanteFaltante) {
          console.log(
            '🔄 Comprobante no existe (404), intentando generar automáticamente...',
          );
  ...
  private cargarClientesRecientes() {
    ...
    this.clienteService.listarActivos().subscribe({
      next: (clientes) => {
        ...
        this.clientesRecientes = clientes
          .sort((a: any, b: any) => {
            const fechaA = new Date(
              a.fechaCreacion || a.createdAt || 0,
            ).getTime();
            const fechaB = new Date(
              b.fechaCreacion || b.createdAt || 0,
            ).getTime();
        ...
        console.log(
          '✅ Clientes recientes cargados:',
          this.clientesRecientes.length,
        );
      },
      error: (error) => {
  ...
  private simularBusquedaPorCodigo(codigo: string): InventarioPOS | null {
    ...
    const producto = this.productosPopulares.find(
      (p) => p.producto?.codigo === codigo || p.codigoCompleto === codigo,
    );
  ...
  // ========================================
  // MÉTODOS DE COMPATIBILIDAD ZEN BOUTIQUE
  // ========================================
  ...
  private abrirCajonDinero(): void {
    this.comprobantesService.abrirCajon().subscribe({
      next: (response) => {
        if (response.success) {
          this.toastService.success('💰 Cajón Abierto', response.message, {
            duration: 2000,
          });
        } else {
          this.toastService.warning('⚠️ Aviso', response.message, {
  ...
  private mostrarConfiguracionPuerto(): void {
    ...
    this.comprobantesService.obtenerPuertosDisponibles().subscribe({
      ...
      error: (error) => {
        ...
        this.toastService.error(
          '❌ Error',
          'No se pudieron obtener los puertos disponibles',
          { duration: 4000 },
        );
      },
    });
  ...
  private configurarPuerto(puerto: string): void {
    this.toastService.info(
      '🔧 Configurando',
      `Configurando ticketera en puerto ${puerto}...`,
      { duration: 2000 },
    );
    ...
    this.comprobantesService.configurarPuertoTicketera(puerto).subscribe({
      next: (response) => {
        if (response.success) {
          this.toastService.success('✅ Puerto Configurado', response.message, {
            duration: 3000,
          });
          ...
          this.toastService.error('❌ Error Configuración', response.message, {
            duration: 4000,
          });
        }
      },
    ...
    this.comprobantesService.verificarConexionTicketera().subscribe({
      next: (response) => {
        ...
        if (response.success) {
          ...
          this.agregarLogPrueba(
            'success',
            `✅ Ticketera conectada en puerto: ${this.estadoConexion.puerto}`,
          );
        } else {
          this.estadoConexion = {
            conectada: false,
          ...
          this.agregarLogPrueba(
            'warning',
            `⚠️ Ticketera desconectada: ${response.message}`,
          );
        }
        this.cdr.markForCheck();
      },
      error: (error) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error verificando conexión: ${error.message || 'Error desconocido'}`,
        );
        this.cdr.markForCheck();
      },
    });
    ...
    this.comprobantesService.imprimirTicketPrueba().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.agregarLogPrueba(
            'warning',
            `⚠️ Problema con ticket de prueba: ${response.message}`,
          );
      ...
      error: (error) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error imprimiendo ticket: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error(
          '❌ Error',
    ...
    this.comprobantesService.imprimirTicketPrueba().subscribe({
      next: (response: any) => {
        if (response.success) {
          this.agregarLogPrueba(
            'success',
            '✅ Texto simple enviado correctamente',
          );
          this.toastService.success(
            '📝 Texto Enviado',
            'Texto simple impreso correctamente',
          );
        } else {
          this.agregarLogPrueba(
            'warning',
            `⚠️ Problema con texto simple: ${response.message}`,
          );
          this.toastService.warning('⚠️ Advertencia', response.message);
        }
      },
      error: (error: any) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error imprimiendo texto: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error(
          '❌ Error',
          'No se pudo imprimir el texto simple',
        );
      },
    });
  }
    ...
    this.comprobantesService.imprimirTicketPrueba().subscribe({
      next: (response: any) => {
        if (response.success) {
          this.agregarLogPrueba(
            'success',
            '✅ Formatos enviados correctamente',
          );
          this.toastService.success(
            '🎨 Formatos Enviados',
            'Prueba de formatos impresa correctamente',
          );
        } else {
          this.agregarLogPrueba(
            'warning',
            `⚠️ Problema con formatos: ${response.message}`,
          );
          this.toastService.warning('⚠️ Advertencia', response.message);
        }
      },
      error: (error: any) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error imprimiendo formatos: ${error.message || 'Error desconocido'}`,
        );
    ...
    this.comprobantesService.cortarPapel().subscribe({
      next: (response) => {
        if (response.success) {
          this.agregarLogPrueba(
            'success',
            '✅ Comando de corte enviado correctamente',
          );
          this.toastService.success(
            '✂️ Papel Cortado',
            'Comando de corte ejecutado correctamente',
          );
        } else {
          this.agregarLogPrueba(
            'warning',
            `⚠️ Problema cortando papel: ${response.message}`,
          );
          this.toastService.warning('⚠️ Advertencia', response.message);
        }
      },
      error: (error) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error cortando papel: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error('❌ Error', 'No se pudo cortar el papel');
      },
    });
    ...
    this.comprobantesService.abrirCajon().subscribe({
      next: (response) => {
        if (response.success) {
          this.agregarLogPrueba(
            'success',
            '✅ Comando de apertura enviado correctamente',
          );
          this.toastService.success(
            '📦 Cajón Abierto',
            'Comando de apertura ejecutado correctamente',
          );
        } else {
          this.agregarLogPrueba(
            'warning',
            `⚠️ Problema abriendo cajón: ${response.message}`,
          );
          this.toastService.warning('⚠️ Advertencia', response.message);
        }
      },
      error: (error) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error abriendo cajón: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error('❌ Error', 'No se pudo abrir el cajón');
      },
    });
    ...
    this.comprobantesService.verificarConexionTicketera().subscribe({
      next: (response: any) => {
        if (response.success && response.data) {
          ...
          this.agregarLogPrueba(
            'success',
            `✅ Estado obtenido: ${estado.estado || 'Conectada'}`,
          );
          this.agregarLogPrueba(
            'info',
            `Puerto: ${estado.puerto || 'Detectado automáticamente'}`,
          );
          this.agregarLogPrueba(
            'info',
            `Conexión: ${response.conectada ? 'Activa' : 'Inactiva'}`,
          );
          ...
          this.agregarLogPrueba(
            'warning',
            `⚠️ No se pudo obtener estado: ${response.message}`,
          );
          this.toastService.warning(
            '⚠️ Advertencia',
            response.message || 'No se pudo obtener el estado',
          );
        }
      },
      error: (error: any) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error obteniendo estado: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error(
          '❌ Error',
          'No se pudo obtener el estado de la ticketera',
        );
      },
    });
  }
    ...
    this.comprobantesService.obtenerPuertosDisponibles().subscribe({
      next: (response) => {
        ...
        if (response.success && response.data) {
          ...
          this.agregarLogPrueba(
            'success',
            `✅ ${this.puertosDisponibles.length} puertos detectados: ${this.puertosDisponibles.join(', ')}`,
          );
          this.toastService.success(
            '🔍 Puertos Detectados',
            `Se encontraron ${this.puertosDisponibles.length} puertos`,
          );
        } else {
          this.puertosDisponibles = [];
          this.agregarLogPrueba(
            'warning',
            '⚠️ No se encontraron puertos disponibles',
          );
          this.toastService.warning(
            '⚠️ Sin Puertos',
            'No se encontraron puertos disponibles',
      ...
      error: (error) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error detectando puertos: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error(
          '❌ Error',
          'No se pudieron detectar los puertos',
        );
        this.cdr.markForCheck();
      },
    });
  ...
  configurarPuertoSeleccionado(): void {
    if (!this.puertoSeleccionado) {
      this.toastService.warning(
        '⚠️ Selecciona Puerto',
        'Debes seleccionar un puerto primero',
      );
      return;
    }
    ...
    this.agregarLogPrueba(
      'info',
      `Configurando puerto: ${this.puertoSeleccionado}`,
    );

    this.comprobantesService
      .configurarPuertoTicketera(this.puertoSeleccionado)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.agregarLogPrueba(
              'success',
              `✅ Puerto ${this.puertoSeleccionado} configurado correctamente`,
            );
            this.toastService.success(
              '🔧 Puerto Configurado',
              `Puerto ${this.puertoSeleccionado} configurado`,
            );
            ...
            this.agregarLogPrueba(
              'warning',
              `⚠️ Error configurando puerto: ${response.message}`,
            );
            this.toastService.warning(
        ...
        error: (error) => {
          ...
          this.agregarLogPrueba(
            'error',
            `❌ Error configurando puerto: ${error.message || 'Error desconocido'}`,
          );
          this.toastService.error(
            '❌ Error',
            'No se pudo configurar el puerto',
          );
        },
      });
  }
  ...
  imprimirVentaActual(): void {
    ...
    this.agregarLogPrueba(
      'info',
      `Imprimiendo venta actual con ${this.carrito.length} productos`,
    );
    this.agregarLogPrueba(
      'warning',
      '⚠️ Para pruebas reales, primero complete una venta',
    );
    this.agregarLogPrueba(
      'info',
      '💡 Use "Imprimir Última Venta" para probar con datos reales',
    );

    // En lugar de crear datos ficticios, sugerirr usar datos reales
    this.toastService.info(
      'ℹ️ Información',
      'Para probar impresión, complete una venta primero y use "Última Venta"',
    );
  }
  ...
  crearVentaPrueba(): void {
    ...
    this.toastService.success(
      '🛒 Venta Simulada',
      'Venta de prueba creada para testing',
    );
    ...
    this.ventasService.obtenerVentasRecientes(1).subscribe({
      next: (ventas: any[]) => {
        if (ventas && ventas.length > 0) {
          ...
          this.agregarLogPrueba(
            'success',
            `✅ Encontrada venta: ${ultimaVenta.numeroVenta}`,
          );
      ...
      error: (error: any) => {
        ...
        this.agregarLogPrueba(
          'error',
          `❌ Error buscando ventas: ${error.message || 'Error desconocido'}`,
        );
        this.toastService.error(
          '❌ Error',
          'No se pudo obtener la última venta',
        );
      },
    });
  }
  ...
  limpiarLogs(): void {
    ...
    this.toastService.info(
      '🗑️ Logs Limpiados',
      'Se limpiaron todos los logs de prueba',
    );
    this.cdr.markForCheck();
  }
  ...
  private async imprimirTicketYPDF(venta: VentaResponse): Promise<void> {
    ...
    try {
      ...
      if (resultadoTicket.status === 'fulfilled') {
        ...
        errores.push(
          `Ticket: ${resultadoTicket.reason?.message || 'Error desconocido'}`,
        );
      }
      ...
      if (resultadoPDF.status === 'fulfilled') {
        ...
        errores.push(
          `PDF: ${resultadoPDF.reason?.message || 'Error al descargar'}`,
        );
      }
      ...
      if (mensajesExito.length > 0) {
        ...
        this.toastService.success('✅ Éxito', mensajesExito.join(' | '), {
          duration: 5000,
        });
      }

      if (errores.length > 0) {
        this.toastService.warning('⚠️ Parcial', errores.join(' | '), {
          duration: 6000,
        });
      }
    } catch (error: any) {
      ...
      this.toastService.error(
        '❌ Error',
        'Error en el proceso de impresión dual',
      );
    } finally {
  ...
  private async imprimirSoloPDF(venta: VentaResponse): Promise<void> {
    try {
      this.loadingImpresion = true;
      this.toastService.info('🔄 Preparando', 'Generando comprobante PDF...', {
        duration: 3000,
      });
      ...
      this.comprobantesService.obtenerComprobantePorVenta(venta.id).subscribe({
        ...
        error: (error) => {
          ...
          if (error.status === 404) {
            this.toastService.info(
              '📝 Generando',
              'Creando comprobante faltante...',
              { duration: 2000 },
            );

            // Generar el comprobante
            this.generarComprobanteCompletoPOS(venta)
              .then((nuevoComprobante) => {
   ...
   */
  private async generarComprobanteCompletoPOS(
    venta: VentaResponse,
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      // Determinar tipo de comprobante basado en documento del cliente
      const tipoComprobante =
        venta.cliente?.documento && venta.cliente.documento.length === 11
          ? 'FACTURA'
          : 'BOLETA';
  ...
  private async imprimirSoloTicket(venta: VentaResponse): Promise<void> {
    ...
    console.log('🔍 Venta completa:', venta);
    console.log(
      '🔍 comprobantesService disponible:',
      !!this.comprobantesService,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        '🔗 URL del endpoint:',
        `/api/comprobantes/venta/${venta.id}/imprimir-ticket`,
      );
  ...
  cancelarPago(): void {
    ...
    if (this.procesandoPago) {
      const confirmar = confirm(
        '⚠️ Se está procesando el pago. ¿Está seguro de cancelar?',
      );
  ...
  isPagoValid(): boolean {
    ...
    if (
      this.pagoActual.metodoPago === 'TARJETA_CREDITO' ||
      this.pagoActual.metodoPago === 'TARJETA_DEBITO'
    ) {
      if (
        !this.pagoActual.nombreTarjeta?.trim() ||
    ...
    if (
      this.pagoActual.metodoPago === 'TRANSFERENCIA' ||
  ...
  getMetodoPagoStyle(
    ...
    > = {
      EFECTIVO:
        'w-8 h-8 bg-green-500 rounded-lg flex items-center justify-center',
      TARJETA_CREDITO:
        'w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center',
      TARJETA_DEBITO:
        'w-8 h-8 bg-purple-500 rounded-lg flex items-center justify-center',
      TRANSFERENCIA:
        'w-8 h-8 bg-indigo-500 rounded-lg flex items-center justify-center',
    ...
    return (
      styles[metodo as keyof typeof styles] ||
      'w-8 h-8 bg-gray-500 rounded-lg flex items-center justify-center'
    );
  }
  ...
  private registrarVenta(): void {
    this.ventasService
      .registrarVenta(this.nuevaVenta)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (venta) => {
          ...
          if (venta.id && venta.numeroVenta) {
            ...
            this.mostrarError(
              'Error inesperado',
              'La venta no se completó correctamente',
            );
            this.procesandoPago = false;
          }
        },
        error: (error) => {
          ...
          if (error.status === 400 && error.error?.message) {
            if (error.error.message.includes('Stock insuficiente')) {
              ...
              console.warn(
                '🔄 Se recargará el inventario debido a conflicto de stock',
              );
            } else {
            ...
            errorMessage =
              'Conflicto de inventario. Verificando estado actual...';
            shouldReload = true;
          } else {
  ...
  private validarVenta(): boolean {
    ...
    if (this.montoPagado < this.totalVenta) {
      this.mostrarError(
        'Pago insuficiente',
        'El monto pagado debe cubrir el total de la venta',
      );
      return false;
    }
  ...
  private async verificarStockTiempoReal(): Promise<boolean> {
    try {
      const verificaciones = this.carrito.map(async (item) => {
        const inventario = await this.inventarioService
          .obtenerInventarioPorId(item.inventarioId)
          .toPromise();

        if (!inventario) {
          console.error(
            `❌ Inventario no encontrado para ID: ${item.inventarioId}`,
          );
          this.mostrarError(
      ...
      .subscribe({
        next: (pago) => {
          ...
          setTimeout(() => {
            this.mostrarExito(
              'Venta procesada',
              `Venta ${venta.numeroVenta} creada exitosamente`,
            );
  ...
  private validarDatosPago(): boolean {
    ...
    if (
      !this.pagoActual.metodoPago ||
      this.pagoActual.metodoPago.trim() === ''
    ) {
  ...
  inicializarEstadoCaja() {
    ...
    console.log(
      '⚠️ inicializarEstadoCaja() obsoleto - usando CajaStateService',
    );
  }
  ...
  cerrarCaja() {
    ...
    this.confirmationService.confirm({
      message:
        '¿Está seguro que desea cerrar la caja? Se volverá a la pantalla inicial.',
    ...
    this.productoService
      ...
      .subscribe({
        next: (response: {
          ...
          this.productos =
            response?.contenido || response?.content || response?.data || [];
  ...
  private cargarInventarios(): void {
    ...
    this.inventarioService
      ...
      .subscribe({
        ...
        error: (error: Error) =>
          this.mostrarError('Error al cargar inventarios', error.message),
      });
  }
}
