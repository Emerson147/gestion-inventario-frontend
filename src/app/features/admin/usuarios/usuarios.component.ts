...
export class UsuariosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    if (this.usuariosTable) {
      this.animationService.headerElementsEntrance(
        [this.usuariosTable.nativeElement],
        {
          distance: 30,
          delay: 0.1,
        },
  ...
  calcularMetricasUsuarios() {
    ...
    const admins =
      this.users?.filter((u) => u.roles?.includes('admin')).length || 0;
    const ventas =
      this.users?.filter((u) => u.roles?.includes('ventas')).length || 0;
    ...
    const nuevosUsuarios =
      this.users?.filter(
        (u) => u.fechaCreacion && new Date(u.fechaCreacion) > treintaDiasAtras,
      ).length || 0;
  ...
  toggleEstadoUsuario(usuario: User): void {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.userService
          .updateUser(usuario.id!, { ...usuario, activo: nuevoEstado })
          .subscribe({
            next: () => {
            ...
            error: (error) =>
              this.handleError(error, `Error al ${mensaje} usuario`),
          });
      },
    });
  }
    ...
    try {
      ...
      const dataToExport = this.selectedUsers.map((user) => ({
        ...
        'Fecha Creación': user.fechaCreacion
          ...
          : 'N/A',
        'Última Actualización': user.fechaActualizacion
      ...
      const workbook = {
        Sheets: { Usuarios_Seleccionados: worksheet },
        SheetNames: ['Usuarios_Seleccionados'],
      };
      ...
      this.showSuccess(
        `${this.selectedUsers.length} usuarios seleccionados exportados correctamente`,
      );
    } catch (error) {
  ...
  aplicarFiltros(): void {
    ...
    if (this.filtroRol) {
      usuarios = usuarios.filter(
        (user) => user.roles && user.roles.includes(this.filtroRol),
      );
    }
  ...
  saveUser(): void {
    ...
    saveOperation!.pipe(finalize(() => (this.loading = false))).subscribe({
      next: () => {
        this.animateSuccess();
        this.hideDialog();
        this.loadUsers();
      ...
      accept: () => {
        ...
        this.userService
          ...
          .subscribe({
            ...
            error: (error) =>
              this.handleError(error, 'No se pudo eliminar el usuario'),
          });
      },
    });
  }
  ...
  private async processMultipleDelete(): Promise<void> {
    ...
    try {
      ...
      this.showSuccess(
        `${this.selectedUsers.length} usuarios eliminados correctamente`,
      );
      this.loadUsers();
      this.selectedUsers = [];
    } catch (error) {
  ...
  onGlobalFilter(
    dt: { filterGlobal: (value: string, matchMode: string) => void },
    event: Event,
  ): void {
  ...
  private animateDialogEntrance(): void {
    setTimeout(() => {
      const formFields = document.querySelectorAll(
        '.p-dialog .flex.flex-col.gap-6 > div',
  ...
  async exportarEstadisticas(): Promise<void> {
    ...
    try {
      ...
      const usuariosRecientesSheet = xlsx.utils.aoa_to_sheet(
        usuariosRecientesData,
      );
  ...
  getEstadisticasAvanzadas() {
    ...
    return {
      ...
      usuariosUltimos7Dias: usuarios.filter(
        (u) => u.fechaCreacion && new Date(u.fechaCreacion) > hace7Dias,
      ).length,
      usuariosConEmail: usuarios.filter((u) => u.email && u.email.trim())
        .length,
      usuariosMultipleRoles: usuarios.filter(
        (u) => u.roles && u.roles.length > 1,
      ).length,
   ...
   */
  private getTopDominiosEmail(
    usuarios: User[],
  ): { dominio: string; count: number }[] {
